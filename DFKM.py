...
class PretrainDoubleLayer(torch.nn.Module):
    ...
    def run(self):
        ...
        for epoch in range(10):
            ...
            print(recons_x)
            m=join(r'test', str(epoch)+'.csv')
            print(m)
            a = recons_x.cpu().detach().numpy()
            ba=pd.DataFrame(a)
            ba.to_csv(m ,sep=',',index=0,header=0)
...
class DeepFuzzyKMeans(torch.nn.Module):
    def __init__(self, X, labels, layers=None, lam=1, sigma=None, gamma=1, lr=10**-3, device=None, batch_size=128):
        ...
        self.sigma = sigma
        self.batch_size = batch_size
        self.n_clusters = 7 #len(np.unique(self.labels)) #确定聚类K个簇
        self.lr = lr
        self._build_up()
    ...
    def run(self):
        ...
        for epoch in range(50):
            ...
            y_pred = y_pred.numpy()
            acc, nmi, ARI= cal_clustering_metric(self.labels, y_pred)
            ...
            # print(type(self.batchlabels))
            # batchlabeltes = np.array(self.batchlabels, dtype=int)
            # batchlabeltes = np.mat(batchlabeltes)
            # print(type(batchlabeltes))
            # print(batchlabeltes.shape)
            # print(batchlabeltes)
            # rbind = np.vstack(y_pred, batchlabeltes)
            # print(rbind)



            print(
                'epoch-{}, loss={}, ACC={}, NMI={}, ARI={}'.format(epoch, loss.item(), acc, nmi, ARI))
            # print(Z)
            m = join(r'test2', str(epoch) + '.csv')
            # print(m)
            a = Z.cpu().detach().numpy()
            ba = pd.DataFrame(a)
            ba.to_csv(m, sep=',', index=0, header=0)
            n = join(r'test3', str(epoch) + '.csv')
            c = y_pred
            ca = pd.DataFrame(c)
            ca.to_csv(n, sep=',', index=0, header=0)
...
if __name__ == '__main__':
    ...
    data, labels = loader.load_data(loader.ORL)
    data = data.T
    for lam in [10**-3]:
